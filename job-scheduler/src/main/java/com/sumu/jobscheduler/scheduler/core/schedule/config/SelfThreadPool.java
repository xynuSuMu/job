 ...
 * @author 陈龙
 * @version 1.0
 * @date 2021-01-21 16:50
 */
public class SelfThreadPool implements ThreadPool {
     ...
     */
    public SelfThreadPool(int threadCount, int threadPriority) {
    ...
    protected void createIncrementWorkerThreads() {
        int planCount = count + 10;
        int realCount = planCount > maxCount ? maxCount : planCount;
        int add = realCount - count;
        for (int i = 1; i <= add; ++i) {
            String threadPrefix = getThreadNamePrefix();
            if (threadPrefix == null) {
                threadPrefix = schedulerInstanceName + "_Worker";
...
//        private BlockingQueue<Runnable> queue = new ArrayBlockingQueue<>(3);
         ...
         * </p>
         */
        WorkerThread(SelfThreadPool tp, ThreadGroup threadGroup, String name,
